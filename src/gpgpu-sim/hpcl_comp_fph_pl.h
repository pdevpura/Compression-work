...
#define MAX_INST_COUNT 20000000
...
public:
  ...
  ~hpcl_comp_fph_pl() {
	...
	cout << " Committed Instructions "<<ins_ct<<"\t"<<m_id<<endl;
	//if(m_cur_state==0)
	//		write_data_file();
 ...
 bool do_sam;
  ...
  mem_fetch* run(long long ins_count,long long iter_sam,long long iter_enc);
...
hufman* hpcl_comp_fph_pl<K>::construct_tree(vector<pair<int,pair<hufman*,string> > >& track)
{
	...
	if(track.size()==1)
	{
		pair<hufman*,string> p = track[0].second;
		if(!p.first) {
			hufman* nd = new hufman("true",p.second);
			return nd;
		} else {
...
void hpcl_comp_fph_pl<K>::fill_data(unordered_map<string,long long> src_track,vector<pair<int,pair<hufman*,string> > >& src_tree,vector<pair<int,string>>& track)
{
	...
	{
		hufman *ptr = NULL;
		src_tree.push_back(make_pair(it->second,make_pair(ptr,it->first)));
		/*
...
template<class K>
mem_fetch* hpcl_comp_fph_pl<K>::run(long long inst_count,long long iter_sam,long long iter_enc)
{
		...
		if(ins_ct < iter_sam) {
			state=0;
					...
					if(do_sam==false) {
						cout << " Reached Max  Sam Instruction Mark, End sampling "<<iter_sam<<endl;
						write_data_file();
						generic_init();
						do_sam=true;
					 }
	...
	for(int i = 0; i < m_pipeline_no; i++) 
	{
		...
		{
			m_comp_pl_proc[i]->run(state,rep_count,t_nibles,exp_track,mh_track,ml_track,exp_tracker,mh_tracker,ml_tracker,m_id);
