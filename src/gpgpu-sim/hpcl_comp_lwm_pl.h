...
void hpcl_comp_lwm_pl<K>::create(unsigned comp_buffer_size) {
  ...
  if(m_pipeline_no == 1) {
    ...
    m_comp_buffer_size = comp_buffer_size;
      ...
      m_comp_pl_proc[0]->set_pl_type(hpcl_comp_lwm_pl_proc<K>::GET_OUTPUT);
      for(unsigned i = 1; i < m_pipeline_no-1; i++)
      {
  	    m_comp_pl_proc[i]->set_pl_type(hpcl_comp_lwm_pl_proc<K>::DUMMY);
      }
      m_comp_pl_proc[m_pipeline_no-1]->set_pl_type(hpcl_comp_lwm_pl_proc<K>::COMP);
...
mem_fetch* hpcl_comp_lwm_pl<K>::run()
{
  ...
  if(m_pipeline_no == 1) {
    ...
    {
      if(m_comp_pl_proc[i]->get_pl_type()==hpcl_comp_lwm_pl_proc<K>::COMP)
      {
	...
	tmp->clean();
      ...
      else if(m_comp_pl_proc[i]->get_pl_type()==hpcl_comp_lwm_pl_proc<K>::GET_OUTPUT)
      {
	hpcl_comp_pl_data *tmp = m_comp_pl_proc[i]->get_output();
	assert(tmp);
