...
public:
  void run(unordered_map<string,int>& word_table,int state,long long& rep_count,long long& t_nibles,unordered_map<string,string> tracker);
  ...
  void update_word_table(string packet,unordered_map<string,int>& word_table);
  enum 
  {
	  NONE=0,
	  DUMMY,
...
template<class K>
void hpcl_comp_sc2_pl_proc<K>::update_word_table(string packet,unordered_map<string,int>& word_table)
{
		for(int i=0;i<packet.size();i+=WORD_LEN)
		{
					cout<<"DEBUG word CT: " << each_word <<endl;
...
string hpcl_comp_sc2_pl_proc<K>::ss_encode(string input,unordered_map<string,string> tracker,float& bits,long long& rep_count)
{
    ...
    for(int i=0;i<input.size();i+=8)
	{
	   ...
	   if(tracker.find(ss)!=tracker.end())
	   {
			#ifdef COMP_DEBUG
			string out_word;
			out_word="0" + tracker[ss];
			int word_sz = 1+tracker[ss].size();
...
// added by abpd
template<class K>
void hpcl_comp_sc2_pl_proc<K>::run(unordered_map<string,int>& word_table,int state,long long& rep_count,long long& t_nibles,unordered_map<string,string> tracker)
{
