#ifndef HPCL_DECOMP_FPH_PL_H_
#define HPCL_DECOMP_FPH_PL_H_

#include "hpcl_decomp_fph_pl_proc.h"
#include <vector>

template <class K>
class hpcl_decomp_fph_pl
{
private:
  unsigned m_id;
  unsigned m_pipeline_no;
  std::vector<hpcl_decomp_fph_pl_proc<K>*> m_decomp_pl_proc;
...
public:
  hpcl_decomp_fph_pl(unsigned pipeline_no, unsigned id);
  ~hpcl_decomp_fph_pl () {}
...
template <class K>
hpcl_decomp_fph_pl<K>::hpcl_decomp_fph_pl(unsigned pipeline_no, unsigned id)
{
...
void hpcl_decomp_fph_pl<K>::create()
{
  for(unsigned i = 0; i < m_pipeline_no; i++) {
    m_decomp_pl_proc.push_back(new hpcl_decomp_fph_pl_proc<K>(m_id));
...
template <class K>
void hpcl_decomp_fph_pl<K>::set_input_data(mem_fetch* data) {
  m_input_data->set_mem_fetch(data);
  m_current_data_cnt++;
}
template <class K>
int hpcl_decomp_fph_pl<K>::get_current_data_cnt() {
  return m_current_data_cnt;
}

/*
template <class K>
void hpcl_decomp_fph_pl<K>::set_input_data(hpcl_comp_pl_data* input_data) {
  m_input_data->copy(input_data);
}
template <class K>
hpcl_comp_pl_data* hpcl_decomp_fph_pl<K>::get_output_data() {
  return m_output_data;
}
template <class K>
hpcl_comp_pl_data* hpcl_decomp_fph_pl<K>::get_input() {
  return m_input_data;
}
*/

template <class K>
void hpcl_decomp_fph_pl<K>::reset_output_data() {
  m_output_data->clean();
}

template <class K>
mem_fetch* hpcl_decomp_fph_pl<K>::run(mem_fetch** fb_mf)
{
  mem_fetch *ret=NULL;
